...
function CurrentPlayer() {
  ...
  for (let i = 0; i < players.length; i++) {
    if (i == currentPlayer) {
      brick += `<div><p style = "color: #346224"> Player: ${players[i].name} Points: ${Point[i].points} </p></div> <div id = "Img${i}"> </div>`;
    } else {
      brick += `<div><p> Player: ${players[i].name} Points: ${Point[i].points} </p></div> <div id = "Img${i}"> </div>`;
    }
  }
...
function checkAnswer() {
  ...
  if (document.querySelector("#checkBox img").id == "cacli" && achievments[9].attchieved == false) {
